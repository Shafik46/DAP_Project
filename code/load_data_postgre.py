...
mongo_collection = mongo_db['EV_Population']
...
try:
    ...
    with open('data.csv', 'r') as f:
        cursor = conn.cursor()
        cursor.copy_expert(
            f"COPY \"Electric_Vehicles\".ev_population FROM STDIN WITH (FORMAT csv, HEADER false, DELIMITER ',')", f)
        conn.commit()
        cursor.execute('SELECT COUNT(*) FROM \"Electric_Vehicles\".ev_population')
